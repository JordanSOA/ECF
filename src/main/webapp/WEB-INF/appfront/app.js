...
loginForm.style = 'style="background-color:#BBBE64;"';
...
loginForm.innerHTML = ` <label for="email" class="label label-default">email: </label>
                        ...
                        <button id="loginBtn"class="btn disabled" type="submit"> SIGN IN </button>`;
...
document.getElementById('loginLink').addEventListener('click', function (evt) {
    ...
    input.email.addEventListener("keyup", function (evt) {
        evt.preventDefault();
    ...
    input.password.addEventListener("keyup", function (evt) {
        evt.preventDefault();
...
const login = function (loginObj) {
    ...
    xhr.onreadystatechange = function () {
        if (this.readyState === 4 && this.status == 200) {
            ...
            divUserProfile.innerHTML = `<div class="container bg-3 text-center" style="background-color:#EAEFBD;width: auto !important;">
                                            <h3>Your Profile</h3>
                                            <label for="fName"> First Name : </label>
                                            <p id="fName">${userObj.firstname}</p>
                                            <label for="lName">Last Name</label>
                                            <p id="lName">Last Name : ${userObj.lastname} </p>
                                            <label for="age">Age : </label>
                                            <p id="age">${age} years </p>
                                        </div>
                                        <div id="message" class="container bg-4 text-center" style="background-color:#EA9010;width: auto !important;">
...
registerForm.style = 'style="background-color:#BBBE64;"';
...
registerForm.innerHTML = ` <label for="email" class="label label-default">email: </label>
                            ...
                            <button id="registerBtn"class="btn disabled" type="submit"> SIGN UP </button>`;
...
document.getElementById('registerLink').addEventListener('click', function (evt) {
    ...
    input.email.addEventListener("keyup", function (evt) {
        ...
        if (input.email.value.trim() < 1 ){
            ...
            if (input.email.value.length < 3) {
                ...
                boolEmailValid = false;
                document.getElementById('registerBtn').className = "btn btn-primary active";
            }
        }
    })
    input.password.addEventListener("keyup", function (evt) {
    ...
    input.firstname.addEventListener("keyup", function (evt){
        evt.preventDefault();
        if (input.firstname.value.trim() < 1 ){
            document.getElementById('registerBtn').className = "btn btn-primary disabled";
        } else {
            if (input.firstname.value.length < 3) {
                document.getElementById('registerBtn').className = "btn btn-primary disabled";
            } else {
                document.getElementById('registerBtn').className = "btn btn-primary active";
            }
    ...
    input.lastname.addEventListener("keyup",function (evt){
        evt.preventDefault();
        if (input.lastname.value.trim() < 1 ){
            document.getElementById('registerBtn').className = "btn btn-primary disabled";
        } else {
            if (input.lastname.value.length < 3) {
                document.getElementById('registerBtn').className = "btn btn-primary disabled";
            } else {
                document.getElementById('registerBtn').className = "btn btn-primary active";
            }
    ...
    input.birthdate.addEventListener("change",function (evt){
        evt.preventDefault();
            if (input.birthdate.value == null || !boolEmailValid) {
                document.getElementById('registerBtn').className = "btn btn-primary disabled";
            } else {
                document.getElementById('registerBtn').className = "btn btn-primary active";
            }
